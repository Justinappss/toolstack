            ...
            <div style={{ maxWidth: 860, margin: "0 auto", padding: "48px 20px 100px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 20, marginBottom: 24 }}>
                    ...
                    <div style={{ padding: "28px", borderRadius: 20, background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.06)" }}>
                        <h2 style={{ fontSize: 16, fontWeight: 800, color: "white", margin: "0 0 24px" }}>Your Sale Details</h2>
                            ...
                            <>
                                ...
                                <div style={{ padding: "28px", borderRadius: 20, background: accentBg, border: `1px solid ${accentBorder}`, textAlign: "center" }}>
                                    ...
                                    <p style={{ fontSize: "clamp(28px, 4vw, 48px)", fontWeight: 900, color: accent, margin: "0 0 6px", letterSpacing: "-0.02em" }}>
                                        {fmt(qty > 1 ? result.total.netPayout : result.single.netPayout)}
                                    </p>
                                ...
                                <div style={{ padding: "24px", borderRadius: 20, background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.06)" }}>
                                    ...
                                    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                                        {[
                                            { label: "Sale price", value: fmt(price), color: "rgba(255,255,255,0.7)" },
                                            { label: `Commission (${isHighValue ? "0" : (category.commissionRate * 100).toFixed(0)}%${isHighValue ? " — high value" : ""})`, value: fmt(result.single.commissionFee), color: accent, minus: true },
                                            { label: "Payment processing (2.9% + $0.30)", value: fmt(result.single.paymentFee), color: "#60a5fa", minus: true },
                                        ].map(row => (
                                        ...
                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                            ...
                                            <span style={{ fontSize: 14, fontWeight: 800, color: "#f87171" }}>−{fmt(result.single.totalFees)}</span>
                                        </div>
                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                            <span style={{ fontSize: 13, fontWeight: 700, color: "rgba(255,255,255,0.7)" }}>Your net payout</span>
                                            <span style={{ fontSize: 14, fontWeight: 800, color: "#34d399" }}>{fmt(result.single.netPayout)}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div style={{ padding: "20px 24px", borderRadius: 16, background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.06)" }}>
                                    ...
                                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                                        <div style={{ padding: "14px", borderRadius: 12, background: accentBg, border: `1px solid ${accentBorder}`, textAlign: "center" }}>
                                            ...
                                            <p style={{ margin: "0 0 2px", fontSize: 18, fontWeight: 900, color: "white" }}>{fmt(result.single.netPayout)}</p>
                                        ...
                                        <div style={{ padding: "14px", borderRadius: 12, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", textAlign: "center" }}>
                                            <p style={{ margin: "0 0 4px", fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.5)" }}>EBAY</p>
                                            <p style={{ margin: "0 0 2px", fontSize: 18, fontWeight: 900, color: "white" }}>{fmt(result.ebay.net)}</p>
                                    ...
                                    {result.single.netPayout > result.ebay.net ? (
                                        <p style={{ margin: "10px 0 0", fontSize: 12, color: "#34d399", fontWeight: 600 }}>
                                            ✓ Whatnot saves you {fmt(result.single.netPayout - result.ebay.net)} per item vs eBay
                                        </p>
                                    ) : (
                                        <p style={{ margin: "10px 0 0", fontSize: 12, color: "rgba(255,255,255,0.4)" }}>
                                            eBay pays {fmt(result.ebay.net - result.single.netPayout)} more per item at this price
                                        </p>
                                    )}
                                </div>
                            </>
                ...
                {result && qty > 1 && (
                    <div style={{ marginBottom: 32, padding: "24px", borderRadius: 20, background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.06)" }}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 12 }}>
                            {[
                                { label: "Gross Revenue", value: fmt(price * qty), color: "rgba(255,255,255,0.8)" },
                                { label: "Commission Fees", value: fmt(result.total.commissionFee), color: accent },
                                { label: "Processing Fees", value: fmt(result.total.paymentFee), color: "#60a5fa" },
                                { label: "Total Fees", value: fmt(result.total.totalFees), color: "#f87171" },
                                { label: "Net Payout", value: fmt(result.total.netPayout), color: "#34d399" },
                            ].map(item => (
                                ...
                                {CATEGORIES.filter((c, i, arr) => arr.findIndex(x => x.commissionRate === c.commissionRate) === i).map((cat, i) => {
                                    ...
                                    return (
                                        <tr key={cat.name} style={{ background: i % 2 === 1 ? "rgba(255,255,255,0.015)" : "transparent", borderTop: "1px solid rgba(255,255,255,0.04)" }}>
                                            ...
                                            <td style={{ padding: "12px 16px", color: "#34d399", fontWeight: 700 }}>{fmt(r.netPayout)}</td>
                                        </tr>
                                    );
                                })}
